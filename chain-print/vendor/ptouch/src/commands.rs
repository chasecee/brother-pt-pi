...
pub trait Commands {
    ...
    /// Select graphics raster mode (PT-P710BT)
    fn select_graphics_raster(&mut self) -> Result<(), Error>;

    /// Transfer raster data using packbits framing (PT-P710BT)
    fn raster_transfer_packbits(&mut self, data: &[u8]) -> Result<(), Error>;
...
impl Commands for PTouch {
    ...
    fn select_graphics_raster(&mut self) -> Result<(), Error> {
        debug!("Select graphics raster mode");
        self.write(&[0x1b, 0x69, 0x52, 0x01], self.timeout)
    }
    ...
    fn raster_transfer_packbits(&mut self, data: &[u8]) -> Result<(), Error> {
        ...
        trace!("Raster transfer (packbits): {:02x?}", &buff[..4 + len]);
        self.write(&buff, self.timeout)
    }
