...
impl PTouch {
    ...
    pub fn new_with_context(o: &Options, context: &Context) -> Result<Self, Error> {
        ...
        // Reset device (libusb reset destabilizes PT-P710BT on macOS)
        ...
        let kernel_active = match handle.kernel_driver_active(interface.number()) {
            Ok(v) => v,
            Err(e) => {
                debug!("kernel_driver_active unavailable: {:?}", e);
                false
            }
        };
        match kernel_active {
    ...
    pub fn print_raw(&mut self, data: Vec<[u8; 16]>, info: &PrintInfo) -> Result<(), Error> {
        if self.device == PTouchDevice::PtP710Bt {
            return self.print_raw_p710(data, info);
        }
